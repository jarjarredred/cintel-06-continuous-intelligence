...
OUTPUT_FILE: Final[Path] = ARTIFACTS_DIR / "system_assessment_jarred.csv"
...
def main() -> None:
    ...
    # Modification: Use pl.when to avoid division by zero if requests == 0
    df = df.with_columns(
        [
            ...
            .alias("error_rate"),
            pl.when(pl.col("requests") > 0)
            .then(pl.col("total_latency_ms") / pl.col("requests"))
            .otherwise(0.0)
